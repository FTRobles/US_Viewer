...
Methods
    ...
    changeInteractorMode(self):
        This method it's activated when the the Anotacion button is pushed, and makes the switch between Image style and polygon style
        to make anotations on the 2D viewer selected
        Input:None
        Output:None
...
Variables
    viewSelected
         This variable help us to know with which view one we are working on
         Values: 0 - Sagital (by default)
                 1 - Axil
                 2 - Sagital
        
    mode
        This variable is ised to know if we are working with a Image Style or a Polygon Style
        Values: 0 - Image Style (by default)
                1 - Polygon Style
...
class Ui_MainWindow(object):
    ...
    def setupUi(self, main_window):
        ...
        self.pushButtonAnotacion = QtWidgets.QPushButton(self.centralwidget)
        self.pushButtonAnotacion.setObjectName("pushButtonAnotacion")
        self.horizontalLayout.addWidget(self.pushButtonAnotacion)       
        ...
        self.pushButtonAnotacion.pressed.connect(lambda: self.changeInteractorMode())
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.pushButtonAnotacion.setText(_translate("main_window", "Anotacion"))
    ...
    def openFileNameDialog(self, type_of_file):
        ...
        if file_name:
            ...
            # The dimensions of the volume are obtained to set up the minimum and maximum of the displacement in X, Y and Z axis this is a list
...
#            change_slice__rot_ui.setSliceRotateDefault(slicerDeafult)
            ...
            #Init change slice rotation widget
            ...
            #Set slicers to each view
                     
            
            #Visualize slices in each frame
            self.viewerSag = change_slice__rot_ui.getViewSliceSagital()
            self.viewerSag.setSlice(sliceImageSagital)
            frame_sag = self.viewerSag.displaySlice(self.centralwidget)
            self.frameSagital.setLayout(frame_sag) 
    
            self.viewerAxi = change_slice__rot_ui.getViewSliceAxial()
            self.viewerAxi.setSlice(sliceImageAxial)
            frame_axi = self.viewerAxi.displaySlice(self.centralwidget)
            self.frameAxial.setLayout(frame_axi)                        
            
            self.viewerCor = change_slice__rot_ui.getViewSliceCoronal()
            self.viewerCor.setSlice(sliceImageCoronal)
            frame_cor = self.viewerCor.displaySlice(self.centralwidget)
    ...
    def changeViewSelected(self, viewSelected):
        change_slice__rot_ui.setViewToMove(viewSelected)
         ...
         # If the view is changed all the views must return to the image style 
        self.viewerSag.setInteractorStyle(0)
        self.viewerAxi.setInteractorStyle(0)
        self.viewerCor.setInteractorStyle(0)
        self.mode = 0
        
        self.viewSelected = viewSelected
        return

    def changeInteractorMode(self):
        # This condition checks if the style mode its Image or Polygon
        if(self.mode == 0):
            # Depending on the view selected the style of this view is changed
            if(self.viewSelected == 0):
                self.viewerSag.setInteractorStyle(1)
            elif(self.viewSelected == 1):
                self.viewerAxi.setInteractorStyle(1)
            elif(self.viewSelected == 2):
                self.viewerCor.setInteractorStyle(1)
            # We set the new mode
            self.mode = 1
